...
def get_artist_top_tracks(auth_header, artist_id, country='US'):
...
def get_users_top(auth_header, type_of_top, time_range, limit):
    if type_of_top not in ['artists', 'tracks']:
        print('invalid type')
        return None
    url = "{}/{type}?time_range={time_range}&limit={limit}".format(USER_TOP_ARTISTS_AND_TRACKS_ENDPOINT, type=type_of_top, time_range=time_range, limit=limit)
    resp = requests.get(url, headers=auth_header)
    #print(resp)
    return resp.json()
...
# https://developer.spotify.com/web-api/get-album-tracks/
def get_album_tracks(auth_header, album_id):
    url = "{}/{id}/tracks?market=US&limit=50".format(GET_ALBUM_ENDPOINT, id=album_id)
    resp = requests.get(url, headers=auth_header)
    return resp.json()

# https://developer.spotify.com/web-api/get-album/
def get_album(auth_header, album_id):
    url = "{}/{id}?market=US".format(GET_ALBUM_ENDPOINT, id=album_id)
    resp = requests.get(url, headers=auth_header)
    return resp.json()
